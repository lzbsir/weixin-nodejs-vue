...
<el-button type="primary">姓名搜索</el-button>
  ...
  <el-table :data="tableData" border style="width:1250px">  
    <el-table-column type="selection" width="55">
      ...
      <el-table-column prop="operation" label="操作">
        ...
        <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)">不同意</el-button>
      </template>
      </el-table-column>
    </el-table>
    </div>
...
export default {
  ...
  // name:goods
mounted(){
    axios.get('http://127.0.0.1:5002/api/returngoods')
      .then(response => {
  ...
  methods:{
    //搜索
    search(vall){
      axios.get('http://127.0.0.1:5002/api/namesearch',{
        params:{neirong:vall}
      })
      .then(response => {
          this.tableData=''
    ...
    handleEdit(index,row){
      console.log(row.id);
       axios.get('http://127.0.0.1:5002/api/agree',{
        params:{id:row.id}
      })
      .then(response => {
    ...
    handleDelete(index,row){
      console.log(row.id);
       axios.get('http://127.0.0.1:5002/api/notagree',{
        params:{id:row.id}
      })
      .then(response => {
        console.log(response.data)
